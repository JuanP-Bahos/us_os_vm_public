...
public class OS {
  ...
  public void interrupt(InterruptType t, Process p, Object i) {
    ...
    switch (t) {
      ...
      case SCHEDULER_CPU_TO_RQ:
        // When the scheduler is preemptive and will send the current process in CPU to the Ready
        // Queue
      ...
      case LOAD_PAGE:
        ...
        if (pmm instanceof PMM_Paging) {
          ...
          cpu.loadPage(
              mpe.getFrameVictim(), mpe.getFrameToLoadFromSwap()); // Bring data from swap to memory
        }
        break;
  ...
  public void create_process(ur_os.process.Process p) {
    ...
    switch (SMM) {
      case PAGING:
        if (p.getSize() == 0) pmm = new PMM_Paging(p, r.nextInt(MAX_PROC_SIZE - 1) + 1, 0);
        else pmm = new PMM_Paging(p, p.getSize(), 0);
        ...
        if (VIRTUAL_MEMORY_MODE_ON) {
          ...
          pmmp.setAssignedPages(
              -1); // The process will get all the pages needed to store the process
        }
        ...
        break;
      case SEGMENTATION:
        if (p.getSize() == 0) pmm = new PMM_Segmentation(r.nextInt(MAX_PROC_SIZE - 1) + 1);
        else pmm = new PMM_Segmentation(p.getSize());
        p.setPMM(pmm);
      ...
      case CONTIGUOUS:
        if (VIRTUAL_MEMORY_MODE_ON) {
          if (p.getSize() == 0)
            pmm =
                new PMM_Contiguous(
                    p, getVMemorySlot(r.nextInt(MAX_PROC_SIZE - 1) + 1), this.lazySwap);
          else pmm = new PMM_Contiguous(p, getVMemorySlot(p.getSize()), this.lazySwap);
        } else {
          if (p.getSize() == 0) {
            ...
            pmm =
                new PMM_Contiguous(
                    p, getVMemorySlot(p.getSize()), getMemorySlot(p.getSize()), this.lazySwap);
        }
  ...
  public void assignFramesToProcess(Process p) {
    ...
    if (VIRTUAL_MEMORY_MODE_ON) {
      pmmp.setFrameID(
          0, freeFrames.getFrame()); // Assign the first page to a frame and it becomes valid
    }
  }
