...
public abstract class FreeMemorySlotManager extends FreeMemoryManager {
  ...
  public int getTotalFreeMemory() {
    int total = 0;
    for (MemorySlot s : list) total += s.getSize();
    return total;
  }

  public int getLargestFreeSlot() {
    int max = 0;
    for (MemorySlot s : list) if (s.getSize() > max) max = s.getSize();
    return max;
  }

  public int getSmallestFreeSlot() {
    int min = Integer.MAX_VALUE;
    for (MemorySlot s : list) if (s.getSize() < min) min = s.getSize();
    return list.isEmpty() ? 0 : min;
  }
  ...
  public double getExternalFragmentation() {
    ...
    return 1.0 - ((double) getLargestFreeSlot() / totalFree);
  }

  // Memory utilization = total free memory / total memory size
  public double getMemoryUtilization(int totalMemory) {
    return (double) getTotalFreeMemory() / totalMemory;
  }
}
